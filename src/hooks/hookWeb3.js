...
export const Web3Functions = () => {
    ...
    const transferMethod = (account, tx, from, token, listOfTransactions, setListOfTransactions) => {
        ...
        setListOfTransactions(listOfTransactions => [
            ...listOfTransactions, 
            createData(rowNumber++, tx.hash, from, _to, web3.utils.fromWei(_value, "ether"), token)
        ]);
    }
    ...
    const transferFromMethod = (account, tx, token, listOfTransactions, setListOfTransactions) => {
        ...
        setListOfTransactions(listOfTransactions => [
            ...listOfTransactions, 
            createData(rowNumber++, tx.hash, _from, _to, web3.utils.fromWei(_value, "ether"), token)
        ]);
    }
    ...
    const searchTransaction = async(tx, account, listOfTransactions, setListOfTransactions) => {
        ...
        if(fromAccount === account)
            setListOfTransactions(listOfTransactions => [
                ...listOfTransactions, 
                createData(rowNumber++, tx.hash, account, toAccount, web3.utils.fromWei(tx.value, "ether"), "ether")
            ]);
    
        else if(toAccount !== null && toAccount === account)
            setListOfTransactions(listOfTransactions => [
                ...listOfTransactions, 
                createData(rowNumber++, tx.hash, fromAccount, account, web3.utils.fromWei(tx.value, "ether"), "ether")
            ]);   
